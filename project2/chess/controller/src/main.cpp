 ...
 * 
 * Last edited: Michael Christensen, 12 June 2012
 * 
...
#include "ChessController.h"	//(ADDED)
...
void connectController(ChessView& view, IChessController ** cont, int argc, char ** argv)
{
	...
	// controller should have pointer to facade (ChessGame object) (ADDED)
	(*cont) = new ChessController(atoi(argv[1]));
 
	// 3. Connect the view and controller using the IChessController::SetView 
	// and IChessView::SetController methods
	view.SetController(*cont);
	(*cont)->SetView(&view);							//REVIEW THESE CONNECTIONS/WHY THEY WORK
}
...
#define IMAGE_PATH "../view/sw_images/"
